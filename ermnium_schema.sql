...
-- TIP POST RPC
CREATE OR REPLACE FUNCTION public.ermnium_tip_post(target_post_id bigint, tip_amount numeric)
RETURNS jsonb AS $$
DECLARE
  tipper_name text;
  author_name text;
  tipper_balance numeric;
BEGIN
  -- Get tipper username
  SELECT username INTO tipper_name FROM public.users WHERE id = auth.uid();
  IF tipper_name IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  IF tip_amount < 1 THEN RAISE EXCEPTION 'Minimum tip is 1 ERN'; END IF;

  -- Get post author
  SELECT username INTO author_name FROM public.posts WHERE id = target_post_id;
  IF author_name IS NULL THEN RAISE EXCEPTION 'Post not found'; END IF;
  IF lower(author_name) = lower(tipper_name) THEN RAISE EXCEPTION 'Cannot tip your own post'; END IF;

  -- Check tipper balance
  SELECT balance INTO tipper_balance FROM ermnium_wallets WHERE username = tipper_name;
  IF tipper_balance IS NULL OR tipper_balance < tip_amount THEN RAISE EXCEPTION 'Insufficient ERN'; END IF;

  -- Ensure author has a wallet
  INSERT INTO ermnium_wallets (username, balance, points_balance) VALUES (author_name, 0, 0) ON CONFLICT (username) DO NOTHING;

  -- Transfer ERN
  UPDATE ermnium_wallets SET balance = balance - tip_amount WHERE username = tipper_name;
  UPDATE ermnium_wallets SET balance = balance + tip_amount WHERE username = author_name;

  -- Update post tip total
  UPDATE public.posts SET tip_total = COALESCE(tip_total, 0) + tip_amount WHERE id = target_post_id;

  -- Log transactions
  INSERT INTO ermnium_transactions (username, type, amount, item_name) VALUES (tipper_name, 'tip_sent', tip_amount, 'Tip to @' || author_name || ' on post #' || target_post_id);
  INSERT INTO ermnium_transactions (username, type, amount, item_name) VALUES (author_name, 'tip_received', tip_amount, 'Tip from @' || tipper_name || ' on post #' || target_post_id);

  RETURN jsonb_build_object(
    'success', true,
    'new_balance', (SELECT balance FROM ermnium_wallets WHERE username = tipper_name),
    'tip_total', (SELECT tip_total FROM public.posts WHERE id = target_post_id)
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
