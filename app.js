...
async function like(postId) {
  ...
  // Re-render just the like count for that post
...
async function loadComments(postId) {
  ...
  const { data: comments } = await sb.from("comments")
    .select("id, username, text, created_at").eq("post_id", postId).order("created_at", { ascending: true });
  ...
  for (const c of (comments || [])) {
    ...
    d.innerHTML =
      ...
      (c.username === currentUser ? ' <span class="del-comment" onclick="delComment(' + c.id + ',' + postId + ')">✕</span>' : '');
    list.appendChild(d);
  }
}
...
async function upload(file) {
  ...
  reader.onload = async () => {
    ...
    // Update right sidebar pic immediately
...
async function render() {
  ...
  const { data: commentCounts } = postIds.length
    ? await sb.from("comments").select("post_id").in("post_id", postIds)
    : { data: [] };
  const commentMap = {};
  ...
  for (const p of (posts || [])) {
    ...
    div.innerHTML =
      '<div class="post-header">' +
        ...
        '</div>' +
        (p.username === currentUser ? '<span class="delete" onclick="del(' + p.id + ')">✕</span>' : '') +
      '</div>' +
  ...
  const rf = document.getElementById("rightFollowers");
  const rb = document.getElementById("rightBio");
  if (rp) rp.src = me.pic || "empty.jpg";
  if (rn) rn.innerHTML = '<a href="userpage.html?user=' + encodeURIComponent(currentUser) + '" style="color:var(--accent);text-decoration:none;">@' + currentUser + '</a>';
